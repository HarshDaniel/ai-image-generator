  ...
  return (
    ...
    <div className='rounded-xl group relative shadow-card hover:shadow-cardhover card'>
      ...
      <div className='group-hover:flex flex-col max-h-[94.5%] hidden absolute bottom-0 left-0 right-0 bg-[#10131f] m-2 p-4 rounded-md'>
        {/* Display the prompt text with white color */}
        <p className='text-white text-sm overflow-y-auto prompt'>{prompt}</p>

        {/* Section with user information and download button */}
        <div className='mt-5 flex justify-between items-center gap-2'>
          {/* Display user avatar and name */}
          <div className='flex items-center gap-2'>
            <div className='w-7 h-7 rounded-full object-cover bg-green-700 flex justify-center items-center text-white text-xs font-bold'>
              {/* Display the first character of the user's name as avatar */}
              {name[0]}
            </div>
            <p className='text-white text-sm'>{name}</p>
          </div>

          {/* Download button with the ability to trigger downloadImage function */}
